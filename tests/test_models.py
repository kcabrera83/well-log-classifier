...
def test_models():
    results = {}
    results['lithology'] = evaluate_model(os.path.join(MODEL_DIR, 'lithology_classifier.pkl'), X, y)
    results['porosity_estimator'] = evaluate_model(os.path.join(MODEL_DIR, 'porosity_estimator.pkl'), X, y)
    return results
